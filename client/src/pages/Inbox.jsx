              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="font-display font-bold text-sm truncate">{other?.name}</span>
                    <span className="font-mono text-xs text-muted shrink-0">{date}</span>
                  </div>
                  <p className={`font-serif text-sm truncate ${conv.unreadCount > 0 ? 'text-ink font-bold' : 'text-muted'}`}>
                    {isMe ? 'You: ' : ''}{last.text}
                  </p>
                </div>
